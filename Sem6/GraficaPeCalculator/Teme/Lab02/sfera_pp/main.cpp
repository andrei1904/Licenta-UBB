...
std::vector<float> sphere(int nMeridiane, int nParalele, float R);
...
int main()
{
    ...
    vertices = sphere(noMeridiane, noParalele, razaSferei);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        view = glm::translate(view, glm::vec3(glm::cos(revolutionAngle), glm::sin(revolutionAngle), -5.0f));
        projection = glm::perspective(glm::radians(-45.0f), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        revolutionAngle += 0.01f;
...
std::vector<float> sphere(int nMeridiane, int nParalele, float R)
{
    ...
    {
        theta1 = M_PI * (-0.5 + (double)(i - 1) / nParalele);

        for (j = 0; j <= nMeridiane; j++)
        {
            phi = 2 * M_PI * (double)(j - 1) / nMeridiane;

            vertex.push_back(R * cos(phi) * cos(theta1));
            vertex.push_back(R * sin(phi) * cos(theta1));
        ...
        theta0 = M_PI * (-0.5 + (double)i / nParalele);
        for (j = 0; j <= nMeridiane; j++)
        {
            double lng = 2 * M_PI * (double)(j - 1) / nMeridiane;

            vertex.push_back(R * cos(lng) * cos(theta1));
            vertex.push_back(R * sin(lng) * cos(theta1));
            vertex.push_back(R * sin(theta1));

            vertex.push_back(R * cos(lng) * cos(theta0));
            vertex.push_back(R * sin(lng) * cos(theta0));
