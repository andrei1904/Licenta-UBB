...
public class MessageTaskController implements Observer<MessageTaskChangeEvent> {
    ...
    public void initialize() {
        ...
        tableColumnData.setCellValueFactory(new PropertyValueFactory<MessageTask, String>("dateString"));
    ...
    public void handleDeleteMessage(ActionEvent actionEvent) {
        MessageTask message = tableView.getSelectionModel().getSelectedItem();

        if (message != null) {
            service.deleteMessageTask(message);
        } else {
    ...
    public void handleUpdateMessage(ActionEvent ev) {
        ...
        if (message != null) {
            showMessageTaskEditDialog(message);
        } else {
            MessageAlert.showErrorMessage(null, "Nu ati selectat niciun mesaj!");
        }
    }
